...
function App() {
  ...
  const loadBookmakers = async () => {
    ...
    try {
      const response = await axios.get(window.Config.getApiUrl(window.Config.endpoints.bookmakers));
  ...
  const loadComplianceDisclaimers = async () => {
    try {
      const response = await axios.get(window.Config.getApiUrl(window.Config.endpoints.compliance));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Make API request
      const response = await axios.post(window.Config.getApiUrl(window.Config.endpoints.convert), formData);
